...
public class FollowWayJudge : MonoBehaviour
{
    public GameObject Flag; // Судейский флажок
    public Transform takeFlag; // Сбор инвентаря
    public MovementWayJudge MyPath; // Используемый путь
    ...
    void Start()
    {
        ...
        transform.LookAt(pointInPath.Current.position); // Объек должен смотреть в сторону следующей точки
    }
    ...
    void Update()
    {
        ...
        if (transform.position == takeFlag.position)
            Flag.SetActive(true); // Когда судья подходит к инвентарю, появляется флажок
    }
}
